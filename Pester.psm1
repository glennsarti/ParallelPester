...
#>
    ...
    begin {
        # If Threads isn't specified default to the numer of logical processors
        if ($Threads -eq 0) { $Threads = (Get-WmiObject -class win32_processor).NumberOfLogicalProcessors }
        $testScripts = @(ResolveTestScripts $Script)

        #** DEBUG
        #$testScripts = $testScripts | Select -First 5
        #** DEBUG
        ...
        $childPesterParams = @{
            ...
            'Show'                         = $Show -band (-bnot [Pester.OutputTypes]::Summary)
        }
        if ($Quiet) {$childPesterParams.Quiet = $true }

        $start = Get-Date

        $thisModulePath = (Get-Module -Name 'Pester').Path

        Write-Verbose "Running Pester with $Threads threads..."
        $results = $testScripts | Invoke-Parallel -Throttle $Threads -Parameter @($childPesterParams, $thisModulePath) -ScriptBlock {
            $testScript = $_
            # Need to make sure we import THIS module explicitly
            Import-Module -Name $parameter[1] -Force | Out-Null

            $outputFile = Join-Path -Path $ENV:TEMP -ChildPath ('ParallelPester_' + [System.IO.Path]::GetRandomFileName())
            $splat = $parameter[0]
            Invoke-Pester -Script $testScript.Path @splat -OutputFormat 'CliXml' -OutputFile $outputFile | Out-Null
            Write-Output (Import-CliXml -Path $outputFile)
            Remove-Item -Path $outputFile -Force -Confirm:$false | Out-Null
        }

        $Finish = Get-Date

        $pester = New-PesterState -TestNameFilter $TestName `
                                  -TagFilter ($Tag -split "\s") `
                                  -ExcludeTagFilter ($ExcludeTag -split "\s") `
                                  -SessionState $PSCmdlet.SessionState `
                                  -Strict:$Strict `
                                  -Show:$Show `
                                  -PesterOption $PesterOption

        # Munge the client pester results into a single result array
        $AllResults = New-Object System.Collections.ArrayList
        $pester.TestActions.Actions = @()
        $results | ForEach-Object {
            ...
            $pester.PendingCount      += $testresult.PendingCount
            $pester.InconclusiveCount += $testresult.InconclusiveCount

            $pester.TestActions.Actions += $testresult.TestActions.Actions

            $testresult.TestResult | ForEach-Object {
                $AllResults.Add($_) | Out-Null
            }
        }
        $pester.TestResult = $AllResults
        $pester.Time = New-TimeSpan -Start $start -End $Finish
...
& $script:SafeCommands['Export-ModuleMember'] New-Fixture, Get-TestDriveItem, Should, Invoke-Pester, Invoke-ParallelPester, Setup, InModuleScope, Invoke-Mock
